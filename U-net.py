...
mask2=cv2.imread('E:/WTERLOO/waterloo_course/DATA_MINING/project/data/nuclei/test_bad_data/mask_2.png',cv2.IMREAD_GRAYSCALE)
mask3=cv2.imread('E:/WTERLOO/waterloo_course/DATA_MINING/project/data/nuclei/test_bad_data/mask_3.png',cv2.IMREAD_GRAYSCALE)
mask4=cv2.imread('E:/WTERLOO/waterloo_course/DATA_MINING/project/data/nuclei/test_bad_data/mask_4.png',cv2.IMREAD_GRAYSCALE)
...
im2=cv2.imread('E:/WTERLOO/waterloo_course/DATA_MINING/project/data/nuclei/test_bad_data/2.png',cv2.IMREAD_COLOR)
im3=cv2.imread('E:/WTERLOO/waterloo_course/DATA_MINING/project/data/nuclei/test_bad_data/3.png',cv2.IMREAD_COLOR)
im4=cv2.imread('E:/WTERLOO/waterloo_course/DATA_MINING/project/data/nuclei/test_bad_data/4.png',cv2.IMREAD_COLOR)
...
XX=np.zeros([670+3,IMG_WIDTH,IMG_HEIGHT,3],dtype='float32')
YY=np.zeros([670+3,IMG_WIDTH,IMG_HEIGHT,1],dtype='float32')
XX[0:670,:,:,:]=x_train
XX[670]=im2
XX[671]=im3
XX[672]=im4
...
checkpointer = ModelCheckpoint('model-dsbowl2018-cheat_128.h5', verbose=1, save_best_only=True)
#results = model.fit(x_train, y_train, validation_split=0.1, batch_size=10, epochs=20, 
                 #   callbacks=[earlystopper, checkpointer])

model = load_model('model_augmented_256256_cheat.h5', custom_objects={'dice_coef_loss': dice_coef_loss,'dice_coef': dice_coef})



ix=0
preds_train = model.predict(x_test[ix:ix+5], verbose=1)

preds_train_t = (preds_train > 0.5).astype(np.float)
#ix = random.randint(0, len(preds_train_t))
ff=x_test[ix].astype('uint8')
cv2.imshow('r',ff)
#cv2.waitKey(0)
